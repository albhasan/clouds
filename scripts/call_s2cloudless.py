...
def subdirs(path):
    ...
    return([os.path.join(path, item) for item in os.listdir(path) if os.path.isdir(path + "/" + item)])



def get_img_dir(path):
    """Given a directory of Sentinel2 images, return a list of paths to the IMG_DATA on each image."""
    img_dirs = subdirs(path)
    img_dirs = [os.path.join(item, "GRANULE") for item in img_dirs if 'GRANULE' in os.listdir(item)]
    img_dirs = [subdirs(item) for item in img_dirs]
    img_dirs = [item for sublist in img_dirs for item in sublist]
    img_dirs = [os.path.join(item, "IMG_DATA") for item in img_dirs if 'IMG_DATA' in os.listdir(item)]
    return(img_dirs)
...
def create_bandarray(path):
    ...
    subdatasets = [os.path.join(path, item) for item in os.listdir(path) if item.endswith("jp2")]
    #
    wms_bands = []
...
def create_mask(band_array, path):
    ...
    arr4d = np.expand_dims(stacked / 10000, 0)  # shape (1, y_pixels, x_pixels, n_bands) # s2cloudless requires binary map
    ...
    #
    template_file = [os.path.join(path, item) for item in os.listdir(path) if item.endswith("01.jp2")][0]
    # save files
